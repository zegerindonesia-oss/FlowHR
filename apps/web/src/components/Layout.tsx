...
export default function Layout() {
    ...
    if (isAdmin) {
        return (
            <div className="gradient-bg text-slate-800 font-display min-h-screen relative flex items-start">
                {/* Decorative blobs */}
    ...
    return (
        <div className="gradient-bg text-slate-800 font-display min-h-screen relative flex justify-center bg-slate-900">
            {/* Mobile Container (max-width for desktop viewing to mimic mobile) */}
            <div className="w-full max-w-md min-h-screen bg-white shadow-2xl relative overflow-hidden flex flex-col">

                {/* Mobile Decorative blobs */}
                <div className="blob-1 !w-64 !h-64 !top-[-50px] !right-[-50px]"></div>
                <div className="blob-2 !w-64 !h-64 !bottom-[-50px] !left-[-50px]"></div>

                {/* Mobile Header */}
                <header className="px-6 py-5 glass-strong flex justify-between items-center z-10 relative border-b border-white/30 sticky top-0">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full border-2 border-white shadow-md bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white font-bold text-lg overflow-hidden relative group">
                            <span className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity"></span>
                            E
                        </div>
                        <div>
                            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-0.5">Welcome back,</p>
                            <h2 className="text-lg font-bold text-slate-800 leading-none">Employee User</h2>
                        </div>
                    </div>
                    <button className="relative p-2.5 bg-white/50 hover:bg-white/70 rounded-full transition-all shadow-sm border border-white/60">
                        <span className="material-symbols-outlined text-[20px] text-slate-700">notifications</span>
                        <span className="absolute top-2 right-2.5 w-2 h-2 bg-accent rounded-full ring-2 ring-white"></span>
                    </button>
                </header>

                {/* Mobile Content Area (scrollable) */}
                <main className="flex-1 overflow-y-auto px-6 py-6 pb-28 relative z-10 custom-scrollbar">
                    <Outlet />
                </main>

                {/* Fixed Bottom Navigation */}
                <nav className="fixed bottom-0 w-full max-w-md glass-strong border-t border-white/40 pb-safe pt-2 px-6 z-50 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                    <ul className="flex justify-between items-center h-16">
                        {[
                            { path: '/', icon: 'home', label: 'Home' },
                            { path: '/attendance', icon: 'nest_clock_farsight_analog', label: 'Clock' },
                            { path: '/messages', icon: 'chat_bubble', label: 'Chat', badge: true },
                            ...
                            <li key={item.path} className="flex-1">
                                <NavLink
                                    ...
                                    className={({ isActive }) =>
                                        `flex flex-col items-center justify-center w-full h-full gap-1 transition-all duration-300 ${isActive ? 'text-primary scale-110' : 'text-slate-400 hover:text-slate-600 hover:-translate-y-1'}`
                                    }
                                >
                                    {({ isActive }) => (
                                        <>
                                            <div className="relative">
                                                <span className={`material-symbols-outlined text-2xl transition-all duration-300 ${isActive ? 'filled-icon drop-shadow-md' : ''}`}>
                                                    {item.icon}
                                                </span>
                                                {item.badge && (
                                                    <span className="absolute -top-1 -right-1.5 w-2.5 h-2.5 bg-gradient-to-r from-accent to-accent-warm rounded-full border-2 border-white"></span>
                                                )}
                                            </div>
                                            <span className={`text-[10px] font-semibold transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}`}>{item.label}</span>
                                            {isActive && (
                                                <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-primary shadow-[0_0_8px_rgba(124,58,237,0.8)]"></div>
                                            )}
                                        </>
                                    )}
                                </NavLink>
